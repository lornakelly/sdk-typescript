...
describe('fieldsOf', () => {
  ...
  it('merges inherited fields rather than making the caller chase the allOf', () => {
    /* 
     * What it tests: You get inherited fields without chasing allOf yourself
     * Example: timeout appears in SetTask and CallHTTP even though it's defined in TaskBase
     */
  ...
  it('names a field type rather than expanding it', () => {
    /* 
     * What it tests: Fields reference types by name, not inline expansion
     * Example: CallHTTP's with field has typeName: "HTTPArguments" but no fields array. Look up HTTPArguments separately to get its 7 fields.
     */
  ...
  it('keeps a field own title and description alongside the reference', () => {
    /* 
     * What it tests: Field-specific descriptions win over generic type descriptions
     * Example: HTTPArguments.endpoint has title "HTTPEndpoint" and description "The HTTP endpoint to send the request to" (specific), while Endpoint type has generic description "Represents an endpoint"
     */
...
describe('definitions', () => {
  ...
  it('is closed — every name it mentions is a key in it', () => {
    /* 
     * What it tests: No dangling references - every typeName can be looked up
     * Example: If HTTPArguments references Endpoint, then Endpoint must be in the registry
     */
  ...
  it('holds a union as its alternatives', () => {
    /* 
     * What it tests: Choice types have variants array
     * Example: Endpoint has 3 variants (RuntimeExpression, UriTemplate, EndpointConfiguration); CallTask has 7 variants (CallAsyncAPI, CallGRPC, etc.)
     */
  ...
  it('reports shared keys as well as alternatives where a position is both', () => {
    /* 
     * What it tests: When a position has both fields AND variants, both are reported
     * Example: RunTask.run has fields await and return PLUS 4 variants (RunContainer, RunScript, RunShell, RunWorkflow)
     */
...
describe('what a dynamic form needs', () => {
  ...
  it('says what goes in an array, via the reference', () => {
    /* 
     * What it tests: Array item types are accessible via reference
     * Example: Container.arguments has typeName: "ContainerArguments", which defines items: { type: "string" }
     */
  ...
  it('distinguishes a map from an object, explicit or implicit', () => {
    /* 
     * What it tests: Maps have keysAreFree: true, objects don't
     * Example: TaskMetadata and ContainerEnvironment are maps (free keys); TaskBaseInput is an object (declared fields)
     */
  ...
  it('marks a position the schema deliberately leaves open', () => {
    /* 
     * What it tests: Positions accepting any JSON have anyValue: true
     * Example: SchemaInline.document accepts any JSON (anyValue: true); SetTask.if doesn't (undefined)
     */
...
describe('enough to build a form from', () => {
  ...
  it.each(['SetTask', 'RunTask', 'CallHTTP'])('leaves nothing undecidable in %s', (typeName) => {
    /* 
     * What it tests: Every position in these tasks can be rendered as a form control
     * Example: Walks entire task structure and maps each position to a control type (const→fixed, enum→select, variants→picker, etc.)
     */
    const registry = definitions();
